...
class GradientPicker {
    constructor(targetInput, options = {}) {
        ...
        this.state = {
            ...
            stops: [{ color: '#4f46e5', position: 0 }, { color: '#9333ea', position: 100 }],
    ...
    init() {
        ...
        document.addEventListener('click', (e) => {
            if (this.isOpen && this.popup && !this.popup.contains(e.target) && e.target !== this.targetInput) {
                this.close();
            }
        });
    ...
    render() {
        ...
        let html = `
            <div class="pgp-tabs">
                <button class="pgp-tab ${this.state.mode === 'gradient' ? 'active' : ''}" data-tab="gradient">Gradient</button>
                <button class="pgp-tab ${this.state.mode === 'solid' ? 'active' : ''}" data-tab="solid">Solid Color</button>
            </div>
        ...
        this.options.presets.forEach((p, i) => {
            const bg = Array.isArray(p) ? `linear-gradient(135deg, ${p[0]}, ${p[1]})` : p;
        ...
        if (this.state.mode === 'gradient') {
            ...
            html += `
                <div class="pgp-row" style="margin-top: 16px;">
                    ...
                    <input type="color" value="${this.hexToRgbInput(this.state.solidColor)}" data-action="solid-input" style="width: 100%; height: 40px; cursor: pointer;">
                </div>
                <div class="pgp-row">
                    <span class="pgp-label">Hex Code</span>
                    <input type="text" value="${this.state.solidColor}" data-action="solid-text" style="width: 80px; padding: 4px; border: 1px solid #d1d5db; border-radius: 4px; text-transform: uppercase;">
                </div>
            `;
        }
        html += `</div>
    ...
    renderStopsList() {
        ...
        this.state.stops.forEach((stop, index) => {
            ...
            div.innerHTML = `
                <input type="color" class="pgp-color-input" value="${this.hexToRgbInput(stop.color)}" data-index="${index}">
                <input type="range" class="pgp-slider" min="0" max="100" value="${stop.position}" data-index="${index}" style="flex:1">
                <span style="font-size:11px; width:28px; text-align:right">${stop.position}%</span>
                ${this.state.stops.length > 2 ? `<button class="pgp-btn-icon" data-action="remove" data-index="${index}">×</button>` : ''}
            `;
            div.querySelectorAll('input').forEach(inp => inp.addEventListener('click', e => e.stopPropagation()));
    ...
    bindEvents() {
        ...
        this.popup.addEventListener('input', (e) => {
            ...
            if (action === 'angle') {
                ...
                this.popup.querySelector('.pgp-label').textContent = `Angle: ${this.state.angle}°`;
            } else if (action === 'solid-input') {
                this.state.solidColor = target.value;
                const textInput = this.popup.querySelector('[data-action="solid-text"]');
                if (textInput) textInput.value = target.value;
            } else if (index !== undefined) {
                if (target.type === 'color') this.state.stops[index].color = target.value;
        ...
        this.popup.addEventListener('click', (e) => {
            ...
            if (action === 'remove') {
                ...
                const lastPos = this.state.stops[this.state.stops.length - 1].position;
                this.state.stops.push({ color: '#888888', position: Math.min(lastPos + 20, 100) });
                this.render();
                ...
                if (Array.isArray(preset)) {
                    ...
                    this.state.stops = [ {color: preset[0], position: 0}, {color: preset[1], position: 100} ];
                } else {
        ...
        const solidText = this.popup.querySelector('[data-action="solid-text"]');
        if (solidText) {
    ...
    getGradientString() {
        ...
        const sorted = [...this.state.stops].sort((a, b) => a.position - b.position);
        return `linear-gradient(${this.state.angle}deg, ${sorted.map(s => `${s.color} ${s.position}%`).join(', ')})`;
    }
    ...
    parseInput(value) {
        ...
        if (value.includes('gradient')) {
            this.state.mode = 'gradient';
            const gradientMatch = value.match(/linear-gradient\(([^,]+)deg,\s*(.+)\)/);
            if (gradientMatch) {
                this.state.angle = parseInt(gradientMatch[1]) || 90;
                const parts = gradientMatch[2].split(/,(?![^(]*\))/).map(s => s.trim());
                this.state.stops = parts.map((part, i) => {
                    const match = part.match(/(.+?)\s+(\d+)%?/);
                    return match ? { color: match[1], position: parseInt(match[2]) } : { color: part, position: (i / (parts.length - 1)) * 100 };
                });
            }
        } else {
            this.state.mode = 'solid';
