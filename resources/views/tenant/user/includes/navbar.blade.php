...
<div class="admin-navbar w-full sticky top-0 right-0 z-99 flex gap-8 items-center justify-between px-4 py-3 bg-primary border-bottom">
    ...
    <div class="search-bar relative hidden md:block text-sm grow">
        <input type="text" placeholder="Search orders, books..."
        ...
        <div class="relative ml-2">
            <button onclick="toggleModel('tenantUserAction')" class="flex items-center gap-2">
                ...
                @else
                    <div class="w-8 h-8 rounded-full bg-hover-secondary border border-primary flex items-center justify-center font-bold text-xs uppercase text-primary">
                        {{ substr($authUser->fname ?? 'U', 0, 1) }}{{ substr($authUser->lname ?? 'S', 0, 1) }}
                    </div>
                @endif
            </button>
            ...
            <div id="tenantUserAction" class="absolute hidden right-0 mt-2 w-64 bg-primary border-primary border-rounded shadow-md overflow-hidden">
                ...
                <div class="px-3 py-3 border-bottom">
                    ...
                    <p class="text-xs text-tertiary font-mono">{{ $authUser->email ?? '' }}</p>
                </div>

                <!-- Custom Portal Links -->
                <div class="py-1">
                    <a href="{{ route('user.dashboard') }}" class="flex items-center gap-2.5 px-3 py-2.5 text-xs font-semibold text-secondary hover:bg-hover-secondary">
                        <i class="fa-solid fa-gauge text-tertiary text-sm w-4"></i>
                        Dashboard Overview
                    </a>
                    <a href="{{ route('user.orders') }}" class="flex items-center gap-2.5 px-3 py-2.5 text-xs font-semibold text-secondary hover:bg-hover-secondary">
                        <i class="fa-solid fa-bag-shopping text-tertiary text-sm w-4"></i>
                        My Orders
                    </a>
                    <a href="{{ route('user.downloads') }}" class="flex items-center gap-2.5 px-3 py-2.5 text-xs font-semibold text-secondary hover:bg-hover-secondary">
                        <i class="fa-solid fa-book-open text-tertiary text-sm w-4"></i>
                        My Downloads
                    </a>
                    <a href="{{ route('user.invoices') }}" class="flex items-center gap-2.5 px-3 py-2.5 text-xs font-semibold text-secondary hover:bg-hover-secondary">
                        <i class="fa-solid fa-file-invoice text-tertiary text-sm w-4"></i>
                        Billing Invoices
                    </a>
                    <a href="{{ route('user.profile') }}" class="flex items-center gap-2.5 px-3 py-2.5 text-xs font-semibold text-secondary hover:bg-hover-secondary">
