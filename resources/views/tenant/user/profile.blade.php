...
@section('title', 'Profile & Settings - Customer Portal')
    ...
    <div class="mb-4 p-4 border-rounded bg-primary border-primary shadow-xs flex flex-wrap justify-between items-center gap-3">
        <div>
            <h1 class="text-xl font-bold text-primary flex items-center gap-2">
                <i class="fa-solid fa-user-gear text-indigo-500"></i> Profile & Security Settings
            </h1>
            <p class="text-xs text-secondary mt-0.5">Update your personal account details, shipping address, and password.</p>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="p-5 border-rounded bg-primary border-primary space-y-4 shadow-xs h-fit">
            <div class="flex flex-col items-center text-center space-y-3">
                <div class="w-20 h-20 rounded-full bg-hover-secondary border-2 border-primary flex items-center justify-center font-bold text-2xl text-primary">
                    {{ substr($user->fname ?? 'U', 0, 1) }}{{ substr($user->lname ?? 'S', 0, 1) }}
                </div>
                <div>
                    ...
                    <span class="inline-block mt-2 px-2.5 py-0.5 rounded text-[10px] font-bold bg-blue-500/10 text-blue-600 border border-blue-500/20 uppercase tracking-wider">
                        Customer Account
                    </span>
                </div>
            </div>
            <div class="pt-3 border-top space-y-2 text-xs">
                <div class="flex justify-between">
                    <span class="text-tertiary">Registered:</span>
                    <span class="text-primary font-mono font-medium">{{ $user->created_at ? $user->created_at->format('d M Y') : 'N/A' }}</span>
                </div>
                <div class="flex justify-between">
                    <span class="text-tertiary">Phone:</span>
                    <span class="text-primary font-mono font-medium">{{ $user->number ?: 'Not provided' }}</span>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-2 p-5 sm:p-6 border-rounded bg-primary border-primary space-y-6 shadow-xs">
            <h3 class="text-sm font-bold text-primary border-bottom pb-3">Update Personal Details</h3>
            ...
            <form action="{{ route('user.profile.update') }}" method="POST" class="space-y-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="text-xs font-semibold text-primary block mb-1">Email Address (Read-only)</label>
                        <input type="email" value="{{ $user->email }}" disabled
                               class="w-full p-3 bg-hover-secondary border-primary border-rounded text-xs text-tertiary cursor-not-allowed">
                    </div>
                    <div>
                        <label class="text-xs font-semibold text-primary block mb-1">Phone Number</label>
                ...
                <div class="space-y-4 pt-2">
                    <h4 class="text-xs font-bold text-primary uppercase tracking-wider">Shipping Address (for Book Deliveries)</h4>
                    <div>
                        <label class="text-xs font-semibold text-primary block mb-1">Street Address</label>
                        <input type="text" name="address" value="{{ old('address', $user->address) }}" placeholder="House/Flat No., Street, Landmark"
                               class="w-full p-3 bg-primary border-primary border-rounded text-xs text-primary input-focus">
                    </div>
                ...
                <div class="space-y-4 pt-4 border-top">
                    <h4 class="text-xs font-bold text-primary uppercase tracking-wider">Change Account Password</h4>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
