<header class="flex items-center justify-between w-full bg-primary px-4 py-2 border-bottom sticky top-0 z-100">
    <div class="flex items-center gap-2 md:flex-1">
        <a href="{{ route('admin.courses.index') }}" class="py-1 px-2 text-lg border-right group mr-2"><i
                class="fa fa-arrow-left group-hover:-translate-x-1 transition-all duration-300"></i></a>
    ...
    <div class="flex-1 flex gap-2 justify-end items-center">
        <x-button class="w-auto" variant="primary" onclick="submitCourseUpdateForm()"
            loadingText="Saving...">Save</x-button>
        <x-button class="w-auto" loadingText="Updating..." variant="primary"
            onclick="document.getElementById('statusForm').submit()">
            {{ ucfirst($course->status) }}
        </x-button>
        <form id="statusForm" action="{{ route('admin.course.status', $course) }}" method="POST" style="display:none;">
            @csrf
            @method('put')
        </form>
    </div>
