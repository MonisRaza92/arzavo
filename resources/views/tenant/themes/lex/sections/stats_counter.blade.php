...
<style>
    ...
    @if(($section->show_divider ?? '0') === '1')
        ...
        .arz-{{ $section->id }} .stats-grid > .lex-stat-item:not(:last-child) {
            position: relative;
        }
        .arz-{{ $section->id }} .stats-grid > .lex-stat-item:not(:last-child)::after {
            content: '';
            position: absolute;
    ...
    @media (max-width: 767px) {
        ...
        @if(($section->show_divider ?? '0') === '1')
            ...
            .arz-{{ $section->id }} .stats-grid > .lex-stat-item::after {
                display: none;
            }
        @endif
    }
