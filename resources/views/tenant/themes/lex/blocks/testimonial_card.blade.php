...
<div {!! $block->attributes() !!} class="lex-testimonial-card arz-border" style="border-radius:{{ $block->border_radius ?? 6 }}px;border-width:{{ $block->border_width ?? 1 }}px;">

    {{-- Decorative quote mark --}}
    <span class="lex-testimonial-quote">"</span>

    @if($showRating)
        <div class="lex-testimonial-stars">
            @for($i = 1; $i <= 5; $i++)
    ...
    <div class="testimonial-author" style="display: flex; align-items: center; gap: 12px; margin-top: 12px;">
        @if($authorImage)
            <img src="{{ image($authorImage) }}" alt="{{ $authorName }}" class="lex-testimonial-avatar">
        @else
...
<style>
    .testimonial-bordered.lex-testimonial-card {
        border: 1px solid var(--arz-border);
    }
    .testimonial-filled.lex-testimonial-card {
        border: none;
        box-shadow: 0 2px 8px rgba(0,0,0,0.06);
    }
    .testimonial-minimal.lex-testimonial-card {
        border: none;
        border-radius: 0;
        border-bottom: 1px solid var(--arz-border);
        padding: 24px 0;
    }
    .testimonial-minimal.lex-testimonial-card:hover {
        transform: none;
        box-shadow: none;
    }
</style>
