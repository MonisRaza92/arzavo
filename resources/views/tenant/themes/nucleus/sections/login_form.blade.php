<div {!! $section->attributes() !!} class="arz-section relative overflow-hidden {{ $section->visibility }}" style="{{ $section->margin . $section->padding }}">
    ...
    <div class="mx-auto {{ $section->form_width ?? 'max-w-md' }} relative z-10">
        <div class="p-8 border-rounded bg-primary border-primary space-y-6">
            ...
            <form action="{{ route_to('login.submit') }}" method="POST" class="space-y-4">
                @csrf
                <div>
                ...
                <div>
                    <div class="flex items-center justify-between mb-1">
                        <label class="text-xs font-bold text-primary">Password</label>
                    </div>
                ...
                <button type="submit" 
                    ...
                    {{ $section->button_text ?? 'Sign In' }} <i class="fa-solid fa-arrow-right text-[11px]"></i>
                </button>
            </form>

            @if($section->show_register_link ?? true)
                <div class="text-center pt-2 border-top text-xs text-secondary">
                    <a href="{{ route_to('register') }}" class="font-bold text-primary hover:underline">
                        {!! html_entity_decode($section->register_link_text ?? "Don't have an account? Create Account") !!}
                    </a>
                </div>
            @endif
        </div>
