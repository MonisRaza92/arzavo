<div id="mediaModal" class="fixed inset-0 bg-black/50 flex justify-center items-start pt-20 hidden z-50">
    <div class="bg-white border-rounded w-11/12 md:w-3/4 lg:w-1/2 max-h-[80vh] overflow-y-auto scrollbar">
        <div class="flex justify-between items-center p-4 border-bottom sticky top-0 left-0 w-full bg-primary">
        ...
        <div class="p-4">
            <div class="flex flex-wrap gap-4">
                ...
                <div class="border-rounded overflow-hidden cursor-pointer" onclick="selectImage('{{ asset($image->filepath) }}', '{{ $image->filename }}')">
                    <img src="{{ asset($image->filepath) }}" alt="{{ $image->filename }}" class="w-auto h-20 object-cover">
                </div>
                @endforeach
                @if ($images->isEmpty())
                <p>No images are available, please upload</p>
                @endif
            </div>
        </div>
        <div class="p-4 sticky bottom-0 left-0 flex justify-end w-full bg-primary">
...
<script>
    ...
    function selectImage(imageUrl, imageName) {
        ...
        // Submit block form (if available)
        if (blockForm && typeof window.submitBlockForm === 'function') {
            // Thoda delay do taaki first request overlap na kare
            setTimeout(() => {
                window.submitBlockForm(blockForm);
            }, 600);
        }
    }
</script>